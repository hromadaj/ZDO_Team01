...
#Funkce
#############################
#Funkce objevuje primky pomoci houhovi trasnformace vstupem je obrazek obsahujici jeden objekt
...
#funkce pro hledani blizkych objektu h,w jsou parametry vyhledavaciho okna height width
...
#Pouze kontrolni funkce pro pripadne odstraneni primky, pokus se v tabulce ji objevuje
...
#Funkce pro hledani uhlu a pruseciku a zaroven take vykresleni a directory pro ulozen obrazku
def process(rezy,stehy,img_og,image_filename,visualize, directory):
  ...
  if visualize == True:
    ...
    save_path = os.path.join(directory, 'processed_'+image_filename)
...
#Podpurna funkce pro hledani pruseciku
...
##############################################################
#Hlavni funkce
#Vstupem je obrazek v sedotonu, puvodni obrazek a informace jestli ma probehnout vizualizace, directory ukazuje kam se ma obrazek ulozit
def main(img,img_og,image_filename,visualize, directory):
    #aplikace Gausovskeho filtru
    ...
    #Konec skeletu a vzberu objektu
    ##################################################################

    
    #Proces hledani vsech stehu
    ##################################################################
    stehy = []
    for objekt in objekty_sy:
        ...
        #Hledani prvniho a posledniho bodu        
        #pouze kontrola jestli je objekt dostatecne velky
        #r je pocet bodu, ktere se aproximuji
        if len(list_hodnot) > 9:
            r = 10
        else:
            r = 1
    ...
    #Konec procesu hledani vsech stehu
    ##################################################################

    #Proces hledani vsech rezu
    ##################################################################
    rezy = []
    ...
    #Konec procesu hledani vsech rezu
    ##################################################################
    ...
    final_stehy = []
    vyhl_okno_x = img.shape[0]/5
    vyhl_okno_y = img.shape[1]/15
    for i in range(len(stehy_deleted)):
        ...
        if kontrola == True:
            ...
            temp = NajdiNearPrimky(primka,stehy_deleted,vyhl_okno_x,vyhl_okno_y)
            #print(len(temp))
            if len(temp)>=1:
                #mergne body
    ...
    if len(rezy) > 1: 
        for i in range(len(rezy_deleted)):
            ...
            if kontrola == True:
                ...
                temp = NajdiNearPrimky(primka,rezy_deleted,vyhl_okno,vyhl_okno)
                #print(len(temp))
                if len(temp)>=1:
                    #mergne body
                    temp = [item for sublist in temp for item in sublist]
    ...
    #Nalezeni pruseciku a vytvoreni vysledku
    intersections,angles = process(final_rezy,final_stehy,img_og,image_filename,visualize, directory)
...
#kontrola zdali se ma vzkreslovat ci nikoliv
if sys.argv[2] == "-v":
    for i in range(3,len(sys.argv)):
        ...
        data_list.append(main(img,img_og,image_filename,True,directory))
elif sys.argv[2] == "-f":
    ...
    for i in range(2,len(sys.argv)):
        ...
        data_list.append(main(img,img_og,image_filename,False, directory))
...
print(f"Zpracovane obrazky se nachazi v: {directory}.")
